...
def LLR(X_train, Y_train, x_test, y_test, c, sol):
    llr = LogisticRegression(C=float(c), solver=sol, penalty='l2', max_iter=200,
                               random_state=0, verbose=False, class_weight='balanced', n_jobs=1)
    llr.fit(X_train, Y_train)

    y_train_pred_prob = llr.predict_proba(X_train)
...
#    print(y_train_pred_prob)
    ...
    y_pred_prob = llr.predict_proba(x_test)
...
#    print(y_pred_prob)
    ...
    print(auc, acc, mcc, CK, Recall, Precision, F1_score, auc_train, acc_train, mcc_train, CK_train, Recall_train,
          Precision_train, F1_score_train)
    return y_test, y_pred_prob, y_pred, auc, acc, mcc, CK, Recall, Precision, F1_score, auc_train, acc_train, \
           mcc_train, CK_train, Recall_train, Precision_train, F1_score_train


def LLR_predict(ifile1, ifile2, ofile1, ofile2, c, sol):
    ...
    y_test, y_pred_prob, y_pred, auc, acc, mcc, CK, Recall, Precision, F1_score, auc_train, acc_train, \
    mcc_train, CK_train, Recall_train, Precision_train, F1_score_train = LLR(X_Train, Y_Train, x_test, y_test, c, sol)

    writer = csv.writer(open(ofile1, "wt", newline=''), delimiter=',')
    writer2 = csv.writer(open(ofile2, "wt", newline=''), delimiter=',')


    newHeaders = ['auc', 'acc', 'mcc', 'CK', 'Recall', 'Precision', 'F1_score', 'auc_train',
...
#    print(y_test)
    ...
    writer2.writerow([auc, acc, mcc, CK, Recall, Precision, F1_score, auc_train,
                      acc_train, mcc_train, CK_train, Recall_train, Precision_train, F1_score_train])
    print("AUC", auc)
    headers = ['Y_true', 'Y_predict_LLR']
